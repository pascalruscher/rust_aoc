...
fn get_contained_pairs(data: Vec<Option<String>>) -> Vec<((i32, i32), (i32, i32))> {
...
fn main() {
    let file = File::open("2022/day04/input.txt").expect("Error on File::open");
    let data = file_to_vec::<String>(file);
    println!("{}", get_contained_pairs(data).len());
}
...
mod tests {
    ...
    fn get_contained_pairs_1() {
        ...
        assert_eq!(get_contained_pairs(data).len(), 2);
    }

    #[test]
    fn get_contained_pairs_2() {
        let data = vec![Some("9-84,7-9".to_string())];
        assert_eq!(get_contained_pairs(data).len(), 0);
    }
}
