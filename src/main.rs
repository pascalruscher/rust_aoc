...
fn main() {
    ...
    if available_years.contains(&args.year.as_str()) == false {
        panic!(
            "Invalid year: {} - Available years are {:?}",
            args.year, available_years
        );
    }
    ...
    let day = if args.day.len() == 2 {
        ...
        if !args.day.contains("a") && !args.day.contains("b") {
            panic!(
                "Invalid day: {} - Day must have a or b as suffix, i.e. 01a",
                args.day
            );
        }
        panic!(
            "Invalid day: {} - Day must have 1 or 2 digits and end with a or b, i.e. 01a",
            args.day
        );
    };
